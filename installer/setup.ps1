...
if ($Uninstall) {
    ...
    Get-ChildItem $base -ErrorAction SilentlyContinue | Where-Object { ($_ | Get-ItemProperty).ElementName -like "Solon vsock *" -or ($_ | Get-ItemProperty).ElementName -like "Monodon vsock *" } | Remove-Item -Force
    exit 0
}

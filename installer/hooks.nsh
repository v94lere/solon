; Hooks NSIS de l'installeur Monodon (Tauri v2, bundle.windows.nsis.installerHooks).
; L'installeur est élevé (installMode perMachine) : c'est ici, et nulle part ailleurs, que Monodon
; touche aux composants Windows et au Gestionnaire de services. L'application, elle, n'est jamais élevée.

!macro NSIS_HOOK_PREINSTALL
  ; Mise à jour par-dessus une installation existante : le service tient monodon-service.exe ouvert.
  ; L'arrêter arrête proprement le moteur (les données de %ProgramData%\Monodon sont conservées).
...
!macro NSIS_HOOK_POSTINSTALL
  DetailPrint "Configuring Monodon (Windows features, service)…"
  ; setup.ps1 : active Hyper-V et la Plateforme de machine virtuelle si nécessaire, installe et
  ; démarre le service MonodonService. Code de retour 3010 = redémarrage requis.
  ...
  ${ElseIf} $0 != 0
    MessageBox MB_ICONEXCLAMATION|MB_OK "Monodon setup failed (code $0).$\r$\nSee %ProgramData%\Monodon\logs\setup.log, then run the installer again."
  ${EndIf}
!macroend

!macro NSIS_HOOK_PREUNINSTALL
  DetailPrint "Stopping and removing the Monodon service…"
  ...
  Pop $0
  MessageBox MB_YESNO|MB_ICONQUESTION|MB_DEFBUTTON2 "Also delete Monodon's container data (images, volumes)?$\r$\nFolder: %ProgramData%\Monodon" /SD IDNO IDNO keep_data
    nsExec::ExecToLog 'powershell -NoProfile -ExecutionPolicy Bypass -Command "Remove-Item -Recurse -Force \"$$env:ProgramData\Monodon\" -ErrorAction SilentlyContinue"'
  keep_data:
!macroend

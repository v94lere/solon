...
docker volume rm -f solon-crashvol 2>$null | Out-Null
docker volume create solon-crashvol 2>&1 | Out-Null
...
$run = docker run -d --name solon-writer -v solon-crashvol:/data $Image sh -c $writer 2>&1
Start-Sleep 3
$state = docker inspect solon-writer --format "{{.State.Status}} {{.State.Error}}" 2>&1
Check "écrivain en marche" ("$state" -match "^running") "run='$run' état='$state'"
$before = (& $Svc exec "cat /var/lib/docker/volumes/solon-crashvol/_data/counter") -join "" | ConvertFrom-Json
...
$after = (& $Svc exec "cat /var/lib/docker/volumes/solon-crashvol/_data/counter; echo; wc -l < /var/lib/docker/volumes/solon-crashvol/_data/log") -join "" | ConvertFrom-Json
...
docker volume rm -f solon-crashvol 2>&1 | Out-Null
Finish

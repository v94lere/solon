...
# service. Ne désinstalle pas : Monodon reste installé comme chez un utilisateur.
# Usage : .\tests\e2e\install-test.ps1 [-Setup <chemin de Monodon_x.y.z_x64-setup.exe>]
param(
    [string]$Setup = (Join-Path $PSScriptRoot "..\..\target\release\bundle\nsis\Monodon_0.1.0_x64-setup.exe")
)
$inst = Join-Path ${env:ProgramFiles} "Monodon"
$env:MONODON_SERVICE_EXE = Join-Path $inst "monodon-service.exe"
. (Join-Path $PSScriptRoot "common.ps1")
$Setup = (Resolve-Path $Setup).Path

# Le service console de développement ne doit pas tenir les pipes.
$dev = Join-Path $Repo "target\release\monodon-service.exe"
...
$winSvc = Get-Service -Name MonodonService -ErrorAction SilentlyContinue
Check "service MonodonService présent" ($null -ne $winSvc) "$($winSvc.StartType)"
if ($winSvc) {
...
Check "monodon-service.exe installé" (Test-Path $Svc) $Svc
$setupLog = Join-Path $env:ProgramData "Monodon\logs\setup.log"
...
if ($s.state -ne "ready") { Get-Content (Join-Path $env:ProgramData "Monodon\logs\monodon-service.log*") -Tail 30 | ForEach-Object { Log "  log | $_" }; Finish }

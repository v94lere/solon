...
docker run -d --name solon-survivor $Image sleep 600 2>&1 | Out-Null

$kill = Join-Path $Build "kill-svc.cmd"
Set-Content -Path $kill -Value "@echo off`r`ntaskkill /F /IM solon-service.exe" -Encoding ascii
...
Check "conteneur toujours en marche" ("$ps" -match "solon-survivor") "$ps"
Check "journal : rattachement" ((Select-String -Path $ConsoleLog -Pattern "rattachement" | Measure-Object).Count -ge 1) ""
docker stop solon-survivor 2>&1 | Out-Null; docker container prune -f 2>&1 | Out-Null
Finish

...
Ces scripts exercent le vrai service (`monodon-service`) contre une vraie machine. Ils exigent une machine
...
|---|---|---|
| `start-console.ps1 -ImageDir <dossier>` | Lance `monodon-service console --start` élevé, journaux dans `.local/build/service-console.log` | UAC ×1 |
...
Prérequis côté CLI Docker : `DOCKER_HOST=npipe:////./pipe/monodon`. Si Docker Desktop est installé, le CLI
...
- `install-test.ps1` : installe Monodon en silence depuis l'installeur NSIS (une fenêtre UAC), vérifie le service Windows réel, démarre le moteur puis enchaîne `e2e.ps1`. Ne désinstalle pas.

- `fresh-machine-phase1.ps1` (élevé) / `fresh-machine-phase2.ps1` : test « machine vierge » : retire Docker Desktop, WSL, Monodon et désactive Hyper-V (phase 1, puis redémarrage, installation de Monodon, redémarrage), puis vérifie Monodon seul (phase 2).

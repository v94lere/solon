...
$name = "monodon-e2e-web"
docker stop $name 2>$null | Out-Null; docker container prune -f 2>$null | Out-Null
$id = docker run -d -p 8080:80 --name $name $Image sh -c "mkdir -p /srv/www && echo bonjour-monodon > /srv/www/index.html && exec httpd -f -p 80 -h /srv/www" 2>&1
...
Check "http://localhost:8080 depuis Windows" ($body -match "bonjour-monodon") "réponse='$($body.Trim())' après $($t.ElapsedMilliseconds) ms"
$st = Status
...
$svcp = Get-Process monodon-service -ErrorAction SilentlyContinue
$vmMB = [math]::Round(($vm | Measure-Object WorkingSet64 -Sum).Sum / 1MB)
$svcMB = [math]::Round(($svcp | Measure-Object WorkingSet64 -Sum).Sum / 1MB)
Log "RAM au repos : $($vm.ProcessName -join ',') working set = $vmMB Mo ; monodon-service = $svcMB Mo ; total = $($vmMB + $svcMB) Mo"

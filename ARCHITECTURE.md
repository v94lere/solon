...
- **Galerie de piles et détection de projet (9 septembre 2026)** : `apps/desktop/src/templates.ts` (catalogue des piles, images officielles via le miroir ECR, suggestions dérivées d'une sonde), `src-tauri/src/stacks.rs` (`stack_probe` : fichiers de projet au premier niveau et modules Odoo ; `project_scaffold` : écriture sans écrasement, chemins confinés au dossier), `components/StackDialog.tsx` (galerie, suggestions, aperçu éditable, création puis ouverture du projet avec démarrage automatique).
- **Logo et charte du 9 septembre 2026** : logo fourni par Valère (hexagone rouge `#E01818`, « S » rose `#F860C0` et rose clair `#F8A8D8`, `apps/desktop/src-tauri/icons/src/solon-source.png` ; carré 1024 avec marge dans `solon.png`, icônes régénérées par `tauri icon`, barre des tâches par `make-state.py`, `src/assets/logo.png`). Charte dérivée : accent **rose** `#F860C0` (liens, boutons, courbes, focus ; texte accentué `#C8258A` en clair, `#F8A8D8` en sombre), nom de la marque en **rouge** `#E01818` (`--brand`), gris légèrement rosés, fond sombre `#120A0F`, rayons portés à 10 / 16 px pour rappeler les formes arrondies du logo. Le rouge n'est pas utilisé pour les actions (réservé au logo) afin de ne pas se confondre avec les erreurs, dont le rouge est distinct (`#B3261E`). Remplace le narval bleu de la veille et la palette bleue.
...
- **Lot « OrbStack » B (7 septembre 2026)** : onglet **Files** (`apps/desktop/src-tauri/src/files.rs`) : liste par `stat` busybox dans la machine sur le système de fichiers fusionné du conteneur ou le dossier du volume ; téléchargement et envoi par l'API `archive`, avec pour les volumes un conteneur auxiliaire jamais démarré (image vide `solon-empty`) ; glisser-déposer depuis Windows.
- **Lot « OrbStack » A (7 septembre 2026)** : onglet **Debug shell** (script `solon-debug` de l'image : boîte à outils dans les espaces de noms du conteneur, via le terminal machine avec commande, `ShellHeader.command`) ; fiche de conteneur **Overview** ; copie de fichiers par l'API `archive` et `tar.exe` de Windows (`container_copy_from` / `container_copy_to`).
...
- **Lot « caractère » (6 septembre 2026)** : (1) **domaines locaux** (`crates/solon-service/src/domains.rs`) : les liaisons de ports portent désormais le nom du conteneur et les étiquettes Compose ; le service en déduit `nom.solon.local` et `service.projet.solon.local` → plus petit port TCP publié, écrit un bloc géré (`# solon-begin … # solon-end`) dans `%SystemRoot%\System32\drivers\etc\hosts` (vidé à l'arrêt du moteur) et sert un mandataire HTTP sur `127.0.0.1:80` routé par l'en-tête `Host` (copie brute ensuite, WebSockets compris ; page 404 listant les domaines ; si le port 80 est pris, `local_domains=false` dans l'état) ; (2) **diagnostic** : commande `diagnostic_export` (crate `zip`) ; (3) **notifications** : `tauri-plugin-notification`, déclenchées côté frontend sur `die` avec code ≠ 0 hors action utilisateur récente, sur passage à `failed`/`degraded`, et sur l'événement `disk_pressure` émis par l'agent (`statvfs` toutes les 60 s, seuil 90 %, réarmé sous 80 %).
- **Lot « dossiers rapides », étape 1 (6 septembre 2026)** : système de fichiers maison **solonfs** monté en parallèle du 9P sous `/mnt/solonfs/<lettre>` pour mesure. Serveur dans le service (`fileserver.rs`, `FS_CONNECTIONS` = 4 connexions ouvertes par l'hôte vers le port vsock **5006** de l'agent — l'hôte reste l'initiateur de toutes les connexions), client **FUSE** dans l'agent (`solonfs.rs`, crate `fuser` 0.15 copiée dans `third_party/` pour corriger la compilation croisée). Protocole `solon_core::fs` : trames `[u32 taille JSON][u32 taille données][JSON][données]`, `Readdir` avec attributs de toutes les entrées, `Lookup` avec attribut complet, lectures ≥ 128 Ko, cache d'attributs/listes 1,5 s côté invité, listages figés par descripteur, droits root 0777/0666 (chmod ignoré). Préchauffage côté hôte des petits fichiers (≤ 256 Ko, ≤ 64 par dossier, 4 en parallèle) au listage, car la première ouverture NTFS coûte ~3,5 ms. Résultats : 4 à 94× plus rapide que 9P selon l'opération (`docs/measurements.md`). Non fait encore : basculer `/mnt/host` (montages des conteneurs) sur solonfs, Windows Famille (retirer la dépendance au périphérique Plan9), client FUSE multi-thread, invalidation du cache par notifications de changement côté hôte.
- **Lot « dossiers rapides », étape 2 (6 septembre 2026)** : bascule. solonfs est monté sur **`/mnt/host/<lettre>`** (chemins utilisés par la traduction des montages et par Compose), le 9P de Windows sur `/mnt/host9p/<lettre>` en secours. Réglage `legacy_file_sharing` (`settings.json`, case dans Réglages) → le service ajoute `solon.fs=9p` à la ligne de commande du noyau et l'agent remet le 9P sur `/mnt/host` sans monter solonfs. Si l'hôte ne fournit aucune connexion pendant 30 s, une opération renvoie EIO au lieu de bloquer le conteneur.
- **Après le test machine vierge (6 septembre 2026)** : (1) `chmod` refusé (EPERM) aux utilisateurs non root des conteneurs sur solonfs → option `DefaultPermissions` retirée du montage FUSE, le système de fichiers arbitre lui-même (tout permis, chmod/chown ignorés) ; (2) `docker compose` introuvable sans Docker Desktop → plugin déclaré via `cliPluginsExtraDirs` ; (3) **l'image du moteur se construit désormais dans un conteneur Solon** (Alpine 3.24, 14 s, dépôt monté par solonfs) au lieu de WSL : les scripts de build évitent le SIGPIPE `curl | head` sous busybox (`pipefail`).
...
1. **Route directe** : le service pose `route add 10.90.0.0 mask 255.255.0.0 <adresse de la machine>` au démarrage du moteur (`crates/solon-service/src/domains.rs`, `ensure_route`) et la retire à l'arrêt. Côté Linux, Docker 28+ protège les conteneurs contre l'accès direct par une règle `raw PREROUTING -d <ip> ! -i <pont> -j DROP` par conteneur (ajoutée en fin de chaîne) et par la politique des ponts dans `FORWARD`. L'agent insère donc **en tête** deux `ACCEPT` pour la passerelle de l'hôte (`172.30.0.1`) : dans `raw PREROUTING` et dans `DOCKER-USER` (`crates/solon-agent/src/net.rs`, `reapply_host_rules`, réappliqué à chaque (re)démarrage de dockerd). Seul l'hôte Windows est concerné : le réseau HNS est en NAT, rien n'entre depuis le LAN.
2. **Domaines** : l'agent émet `EndpointsChanged` (id, nom, étiquettes Compose, adresse, ports TCP exposés par l'image et publiés) à chaque changement ; le service en déduit `nom.solon.local` et `service.projet.solon.local` → `ip:port`, le port étant le premier exposé parmi 80, 8080, 3000, 8000, 8069, 5000…, sinon le premier exposé, sinon le premier publié, sinon 80. Bloc géré du fichier `hosts` vers `127.0.0.1` ; mandataire `Host`/SNI sur `127.0.0.1:80` et `127.0.0.1:443` qui relaie vers l'adresse du conteneur par la route ci-dessus (page 502 explicite si le conteneur n'écoute pas sur ce port).
3. **HTTPS de confiance** : autorité locale « Solon Local CA » (`rcgen`, clé ECDSA P-256, 2026→2036) créée une fois dans `%ProgramData%\Solon\ca\` (lisible par SYSTEM et les administrateurs seulement) et installée dans le magasin racine machine par `certutil -addstore Root` ; certificats feuilles signés à la volée par nom (SNI, `rustls` `ResolvesServerCert`, cache mémoire). Edge, Chrome, Firefox (magasin Windows) et .NET acceptent ; `curl.exe` (Schannel) exige `--ssl-no-revoke` car les feuilles n'ont pas de point de révocation, comme avec mkcert.

...
| Accès API Docker | Le service Windows relaie l'API Docker de HvSocket vers un **named pipe** `\\.\pipe\monodon` ; `bollard` s'y connecte ; bonus : compatible CLI `docker` | Transport `bollard` custom directement sur HvSocket (gardé en option) | Haute |
...
| Privilèges | **Service Windows `MonodonService`** (LocalSystem) possède la VM ; l'app Tauri lui parle par named pipe ; l'installeur fait toute l'élévation | App élevée en permanence (UAC à chaque lancement) | Haute |
...
Conséquences : (a) tous les prérequis de la piste HCS sont déjà réunis ici, le spike peut démarrer immédiatement ; (b) Docker Desktop cohabite : Monodon ne doit jamais toucher au pipe `docker_engine`, ni aux réseaux HNS de Docker Desktop, ni au contexte `docker` par défaut de l'utilisateur ; (c) la machine ne permet **pas** de tester le cas Windows Home ni le cas « fonctionnalités désactivées » sans manipulation volontaire (à faire dans une VM Hyper-V imbriquée ou sur une seconde machine).
...
- Une VM HCS en boot direct du noyau démarre depuis Rust (`monodon-vm-hcs`, bindings `windows` 0.62) : création 51 ms, `HcsStartComputeSystem` 29 ms, **espace utilisateur atteint ~0,7 s** après l'ordre de démarrage, arrêt propre détecté par l'événement `HcsEventSystemExited` avec un document JSON (`ExitType`, `Initiator`) qui distingue arrêt propre et crash.
...
- Le noyau WSL2 a en dur tout ce dont Monodon a besoin (hv_sock, 9P, balloon, page reporting, squashfs, overlay) sauf `BRIDGE` et `EROFS` (modules) : notre configuration les passera en `=y`. Le noyau Alpine `linux-virt` démarre aussi mais ses pilotes Hyper-V sont en modules : plan B confirmé, pas mieux.
...
- Pipeline `image/build.sh` opérationnel : noyau `6.18.40.1-monodon` compilé depuis le dépôt WSL2 (fragment `monodon.config`, familles netfilter en dur), rootfs Alpine v3.24 avec Docker Engine 29.5.3 en VHD fixe de 293 Mo, initrd de 0,6 Mo, manifeste avec SHA-256.
- **Moteur Docker prêt ~1,2 s après l'ordre de démarrage** (initrd 0,69 s, agent PID 1 0,76 s, dockerd 1,16 s), `docker version` depuis le CLI Windows via `\\.\pipe\monodon` en 125 ms, `docker run --rm` en ~620 ms. Persistance du disque de données validée sur deux cycles, arrêt propre en ~0,4 s.
...
- **Service `monodon-service` opérationnel** (mode service Windows et mode console) : machine à états complète (§7.1), prérequis, image vérifiée par SHA-256, disque de données VHDX créé par `CreateVirtualDisk`, détection d'orphelines et rattachement, réseau HNS, machine, agent, configuration réseau de l'invité, attente de dockerd. Canal de contrôle `\\.\pipe\monodon-control` et API Docker `\\.\pipe\monodon` accessibles **sans élévation**.
...
- **Sécurité** : pipes `\.\pipe\monodon` et `monodon-control` accessibles aux utilisateurs **interactifs** (IU) et administrateurs seulement, plus au groupe « utilisateurs authentifiés » ; le service ne fait toujours aucune requête sortante.
...
- **Installeur** : image et service installés à côté de l'exécutable (`<install>\image`, `<install>\monodon-service.exe`), le service la trouve sans copie ; `installer/setup.ps1` (élevé) active les composants Windows, enregistre les GUID HvSocket et installe `MonodonService` ; désinstallation avec question avant de supprimer `%ProgramData%\Monodon`.
...
- Livrables : `README.md` (prérequis, installation, dépannage par code), `CONTRIBUTING.md`, `LICENSE` (Apache-2.0), ce document, `docs/measurements.md`, installeur NSIS `Monodon_0.1.0_x64-setup.exe` (80 Mo) construit et testé (§1.9), chaîne de signature prête mais non exercée (pas de certificat).
- **Logo narval (8 septembre 2026)** : dessin de Valère (`apps/desktop/src-tauri/icons/src/narwhal-source.png`, silhouette noire, fond transparent), recadré au carré avec 12 % de marge et recoloré en bleu `#3C82C3` (`narwhal.png`, 1024 px) pour rester lisible sur les deux thèmes ; toutes les icônes régénérées par `tauri icon`, icônes de barre des tâches par `make-state.py`, logo de la barre latérale `src/assets/logo.png` (256 px). Le cube est retiré.
- **Palette révisée (7 septembre 2026, soir)** : couleurs principales **noir, blanc et bleu `#3C82C3`** (demande de Valère). Fond blanc / noir, texte noir / blanc, accent bleu partout (liens, boutons, courbes, barre d'accent du menu, curseur des terminaux) ; logo cube recoloré en bleu à luminosité égale (`icons/src/icon.svg`, `src/assets/logo.svg`, icônes régénérées par `tauri icon`) ; verts, ambres et rouges d'état conservés, le vert néon remplacé par un vert standard. L'option « couleur d'accent de Windows » reste disponible.
- **Charte graphique fournie par l'utilisateur (3 septembre 2026, soir)** : police Urbanist (SIL OFL, fichier variable embarqué dans l'application, aucun chargement réseau), blanc et `#F1F1F1` en clair, `#222222` en sombre, accent orange `#FF7A47` (boutons principaux, section active, liens de ports), vert `#66FF4C` pour les points « en marche » (texte en vert plus sombre pour le contraste), logo cube décliné en orange.
...
- **Lot du 3 septembre 2026 (soir)** : (1) **vue par projet Compose** (liste récents + détectés via l'étiquette `com.docker.compose.project.working_dir` traduite en chemin Windows ; écran par projet : services, journaux mêlés — un flux `logs` par conteneur préfixé du nom de service —, Up / Down / Rebuild, Explorateur, VS Code via `code` puis `vscode://`) ; (2) **terminal dans la machine** : l'agent écoute le vsock **5004**, ouvre un pseudo-terminal (`openpty`) et lance `/bin/sh -l` ; hôte → agent : en-tête JSON puis trames `[type, len16, charge]` (saisie / redimensionnement) ; agent → hôte : octets bruts ; le service relaie le pipe `\\.\pipe\monodon-shell` ; l'application le présente dans une modale xterm.js (`Ctrl+\``) ; (3) **CLI intégré** : `crates/monodon-docker-shim` produit `bin\docker.exe`, lanceur du CLI Docker officiel (`docker-cli.exe`, téléchargé par `installer/fetch-docker-cli.ps1`, redistribué sous Apache-2.0) avec `DOCKER_HOST` Monodon par défaut ; le plugin Compose livré est déclaré dans `cliPluginsExtraDirs` du `config.json` de l'utilisateur (le CLI n'a pas de variable d'environnement pour les dossiers de plugins ; découvert lors du test machine vierge, où le plugin de Docker Desktop masquait l'absence du nôtre) ; `setup.ps1` ajoute `<install>\bin` en tête du PATH machine (retiré à la désinstallation) ; (4) **recherche globale Ctrl+K** (palette : sections, actions moteur, conteneurs, images, volumes, réseaux, projets ; `Ctrl+1…6`) ; (5) **processeurs par défaut = cœurs logiques − 2** (au moins 2), calculé par le service quand `settings.json` est absent.
- **Lot « trois défauts » (5–6 septembre 2026)** : (1) le relais brut du pipe Docker est remplacé par un **mandataire HTTP** (`crates/monodon-service/src/docker_proxy.rs`) qui lit les requêtes du client, réécrit `HostConfig.Binds` et `HostConfig.Mounts[].Source` de `POST /containers/create` (`C:\…`, `C:/…`, `/c/…` → `/mnt/host/c/…`), partage le lecteur à la volée, transmet tels quels les corps `Content-Length` et `chunked` et passe en copie brute après un `Connection: Upgrade` (hijack) ; testé avec `docker run -v`, `--mount`, `docker compose` du CLI Windows et `docker exec` ; (2) **exécution en flux** : vsock **5005** dans l'agent (`sh -c`, trames stdout/stderr/exit), pipe `\.\pipe\monodon-exec`, commande `compose_stream` et sortie en direct dans l'écran projet ; (3) **mémoire** : l'agent vide le cache et **compacte** la mémoire toutes les 30 s au repos (`compact_memory`), pour former les blocs de 2 Mo que le ballon Hyper-V peut rendre à l'hôte.
- **Réveil à la demande (7 septembre 2026, soir)** : `crates/monodon-service/src/sleep.rs`. Le mandataire des domaines et les relais de ports signalent chaque connexion (`on_connection_ip` / `on_connection_name`, garde vivante le temps de la connexion) ; une boucle met en pause (`docker pause` via `Command::Exec`) les conteneurs déjà joints via Monodon, sans connexion ouverte, inactifs au-delà du délai, hors liste « toujours éveillé », et seulement s'ils sont `running` ; la connexion suivante fait `docker unpause` avant le relais. Les événements Docker (`unpause`, `stop`…) sortent le conteneur de la liste. Réglages `sleep_enabled`, `sleep_idle_minutes`, `sleep_never`, appliqués sans redémarrage ; `EngineSnapshot.sleeping` pour l'interface.
- **Lot « visuel » (7 septembre 2026, soir)** : briques partagées `apps/desktop/src/components/ui.tsx` (en-tête de page, état vide, squelettes, pastille d'initiale, courbe, mini-jauge), bloc moteur avec mesures (`metrics.ts`), couleur d'accent Windows (`accent.ts`, commande `system_accent_color`), icône de barre des tâches par état, état de fenêtre mémorisé.
- **Lot « OrbStack » B (7 septembre 2026)** : onglet **Files** (`apps/desktop/src-tauri/src/files.rs`) : liste par `stat` busybox dans la machine sur le système de fichiers fusionné du conteneur ou le dossier du volume ; téléchargement et envoi par l'API `archive`, avec pour les volumes un conteneur auxiliaire jamais démarré (image vide `monodon-empty`) ; glisser-déposer depuis Windows.
- **Lot « OrbStack » A (7 septembre 2026)** : onglet **Debug shell** (script `monodon-debug` de l'image : boîte à outils dans les espaces de noms du conteneur, via le terminal machine avec commande, `ShellHeader.command`) ; fiche de conteneur **Overview** ; copie de fichiers par l'API `archive` et `tar.exe` de Windows (`container_copy_from` / `container_copy_to`).
- **Lot « adresses » et lot « menu » (6 septembre 2026, soir)** : voir `docs/measurements.md`. Réseau : route hôte → `10.90.0.0/16`, règles `ACCEPT` de l'agent (raw `PREROUTING`, `DOCKER-USER`), domaines pour tout conteneur, HTTPS par autorité locale (§6). Application : menu en deux catégories (Docker / General), repli en icônes (`Ctrl+B`), Projets fusionnés dans Conteneurs, section Terminal persistante, écran **Activity** alimenté par `Command::Metrics` de l'agent (compteurs bruts, l'application dérive pourcentages et débits) et par le flux `stats` de Docker.
- **Lot « caractère » (6 septembre 2026)** : (1) **domaines locaux** (`crates/monodon-service/src/domains.rs`) : les liaisons de ports portent désormais le nom du conteneur et les étiquettes Compose ; le service en déduit `nom.monodon.local` et `service.projet.monodon.local` → plus petit port TCP publié, écrit un bloc géré (`# monodon-begin … # monodon-end`) dans `%SystemRoot%\System32\drivers\etc\hosts` (vidé à l'arrêt du moteur) et sert un mandataire HTTP sur `127.0.0.1:80` routé par l'en-tête `Host` (copie brute ensuite, WebSockets compris ; page 404 listant les domaines ; si le port 80 est pris, `local_domains=false` dans l'état) ; (2) **diagnostic** : commande `diagnostic_export` (crate `zip`) ; (3) **notifications** : `tauri-plugin-notification`, déclenchées côté frontend sur `die` avec code ≠ 0 hors action utilisateur récente, sur passage à `failed`/`degraded`, et sur l'événement `disk_pressure` émis par l'agent (`statvfs` toutes les 60 s, seuil 90 %, réarmé sous 80 %).
- **Lot « dossiers rapides », étape 1 (6 septembre 2026)** : système de fichiers maison **monodonfs** monté en parallèle du 9P sous `/mnt/monodonfs/<lettre>` pour mesure. Serveur dans le service (`fileserver.rs`, `FS_CONNECTIONS` = 4 connexions ouvertes par l'hôte vers le port vsock **5006** de l'agent — l'hôte reste l'initiateur de toutes les connexions), client **FUSE** dans l'agent (`monodonfs.rs`, crate `fuser` 0.15 copiée dans `third_party/` pour corriger la compilation croisée). Protocole `monodon_core::fs` : trames `[u32 taille JSON][u32 taille données][JSON][données]`, `Readdir` avec attributs de toutes les entrées, `Lookup` avec attribut complet, lectures ≥ 128 Ko, cache d'attributs/listes 1,5 s côté invité, listages figés par descripteur, droits root 0777/0666 (chmod ignoré). Préchauffage côté hôte des petits fichiers (≤ 256 Ko, ≤ 64 par dossier, 4 en parallèle) au listage, car la première ouverture NTFS coûte ~3,5 ms. Résultats : 4 à 94× plus rapide que 9P selon l'opération (`docs/measurements.md`). Non fait encore : basculer `/mnt/host` (montages des conteneurs) sur monodonfs, Windows Famille (retirer la dépendance au périphérique Plan9), client FUSE multi-thread, invalidation du cache par notifications de changement côté hôte.
- **Lot « dossiers rapides », étape 2 (6 septembre 2026)** : bascule. monodonfs est monté sur **`/mnt/host/<lettre>`** (chemins utilisés par la traduction des montages et par Compose), le 9P de Windows sur `/mnt/host9p/<lettre>` en secours. Réglage `legacy_file_sharing` (`settings.json`, case dans Réglages) → le service ajoute `monodon.fs=9p` à la ligne de commande du noyau et l'agent remet le 9P sur `/mnt/host` sans monter monodonfs. Si l'hôte ne fournit aucune connexion pendant 30 s, une opération renvoie EIO au lieu de bloquer le conteneur.
- **Après le test machine vierge (6 septembre 2026)** : (1) `chmod` refusé (EPERM) aux utilisateurs non root des conteneurs sur monodonfs → option `DefaultPermissions` retirée du montage FUSE, le système de fichiers arbitre lui-même (tout permis, chmod/chown ignorés) ; (2) `docker compose` introuvable sans Docker Desktop → plugin déclaré via `cliPluginsExtraDirs` ; (3) **l'image du moteur se construit désormais dans un conteneur Monodon** (Alpine 3.24, 14 s, dépôt monté par monodonfs) au lieu de WSL : les scripts de build évitent le SIGPIPE `curl | head` sous busybox (`pipefail`).
...
```
image/<version>/
├── vmlinuz           noyau Linux, bzImage (15 Mo mesurés, 6.18.40.1-monodon)
...
Base : **Alpine Linux** (version épinglée, `alpine-minirootfs`). Paquets : `docker-engine`, `docker-cli`, `docker-cli-compose`, `containerd`, `runc`, `iptables`/`nftables`, `e2fsprogs`, `ca-certificates`, `busybox`, et notre binaire `monodon-agent` (Rust, cible `x86_64-unknown-linux-musl`, statique). Retirés : `apk` lui-même après build, docs, locales, `openrc` (l'agent est PID 1), tout service réseau (`sshd`, `chrony` : l'horloge est fournie par `hv_utils`).
...
1. `Dockerfile.kernel` : clone du tag, application de `monodon.config`, `make -j bzImage`, sortie `vmlinuz`.
2. `Dockerfile.rootfs` : `apk --root ... add` des paquets épinglés par version exacte (`APKINDEX` figé), copie de `monodon-agent`, nettoyage, `mkfs.ext4 -d` d'une image brute, conversion en VHDX par `qemu-img convert -O vhdx` (ou squashfs si le gain le justifie, décidé au spike).
...
Le service ouvre `\\.\pipe\monodon` (ACL : SYSTEM + utilisateur interactif) et relaie chaque connexion vers le port 5001 de la VM, y compris les connexions hijackées (`exec`/`attach`). Avantages : (1) `bollard` utilise `connect_with_named_pipe`, zéro code exotique dans l'app ; (2) l'utilisateur peut faire `docker context create monodon --docker host=npipe:////./pipe/monodon` avec son CLI existant ; (3) c'est le point unique où appliquer la **réécriture des chemins Windows** dans `POST /containers/create` (`Binds`, `Mounts`) : `C:\Users\v\proj` → `/mnt/host/c/Users/v/proj`, en déclenchant au passage le montage du lecteur s'il ne l'est pas. `bollard::Docker::connect_with_custom_transport` reste l'option si l'on veut supprimer un saut plus tard.
...
- Interface Rust `HostShareProvider { fn expose(&self, host_path) -> Result<GuestPath>; fn release(...); fn stats(...) }` implémentée par `Plan9ShareProvider`. Les implémentations futures (`SyncShareProvider`, `MonodonFsProvider`) ne changent ni l'agent ni l'UI.
...
- Message honnête dans l'UI (sans mentionner de VM) : « Les fichiers Windows montés sont plus lents que les volumes Monodon. Pour les dépendances et les bases de données, préférez un volume. »
...
**Adresses qui marchent, toujours (6 septembre 2026).** Chaque conteneur en marche est joignable depuis Windows sans publier de port, de trois façons complémentaires :
1. **Route directe** : le service pose `route add 10.90.0.0 mask 255.255.0.0 <adresse de la machine>` au démarrage du moteur (`crates/monodon-service/src/domains.rs`, `ensure_route`) et la retire à l'arrêt. Côté Linux, Docker 28+ protège les conteneurs contre l'accès direct par une règle `raw PREROUTING -d <ip> ! -i <pont> -j DROP` par conteneur (ajoutée en fin de chaîne) et par la politique des ponts dans `FORWARD`. L'agent insère donc **en tête** deux `ACCEPT` pour la passerelle de l'hôte (`172.30.0.1`) : dans `raw PREROUTING` et dans `DOCKER-USER` (`crates/monodon-agent/src/net.rs`, `reapply_host_rules`, réappliqué à chaque (re)démarrage de dockerd). Seul l'hôte Windows est concerné : le réseau HNS est en NAT, rien n'entre depuis le LAN.
2. **Domaines** : l'agent émet `EndpointsChanged` (id, nom, étiquettes Compose, adresse, ports TCP exposés par l'image et publiés) à chaque changement ; le service en déduit `nom.monodon.local` et `service.projet.monodon.local` → `ip:port`, le port étant le premier exposé parmi 80, 8080, 3000, 8000, 8069, 5000…, sinon le premier exposé, sinon le premier publié, sinon 80. Bloc géré du fichier `hosts` vers `127.0.0.1` ; mandataire `Host`/SNI sur `127.0.0.1:80` et `127.0.0.1:443` qui relaie vers l'adresse du conteneur par la route ci-dessus (page 502 explicite si le conteneur n'écoute pas sur ce port).
3. **HTTPS de confiance** : autorité locale « Monodon Local CA » (`rcgen`, clé ECDSA P-256, 2026→2036) créée une fois dans `%ProgramData%\Monodon\ca\` (lisible par SYSTEM et les administrateurs seulement) et installée dans le magasin racine machine par `certutil -addstore Root` ; certificats feuilles signés à la volée par nom (SNI, `rustls` `ResolvesServerCert`, cache mémoire). Edge, Chrome, Firefox (magasin Windows) et .NET acceptent ; `curl.exe` (Schannel) exige `--ssl-no-revoke` car les feuilles n'ont pas de point de révocation, comme avec mkcert.
...
Uninitialized
 ...
 → PreparingImage (vérif. SHA-256, copie vers %ProgramData%\Monodon\image\<v>)
...
Chaque transition est journalisée (`tracing`, fichiers tournants dans `%ProgramData%\Monodon\logs`) et diffusée à l'UI par événements. Chaque `Failed` porte un **code stable** documenté dans le README.

### 7.2 Détection des prérequis et messages actionnables

| Détection | Source | Message utilisateur (FR, sans « VM ») |
|---|---|---|
| Virtualisation matérielle désactivée | `Win32_ComputerSystem.HypervisorPresent=false` et `Win32_Processor.VirtualizationFirmwareEnabled=false` | « La virtualisation matérielle est désactivée dans le BIOS/UEFI. Activez « Intel VT-x » ou « AMD-V » (souvent dans l'onglet Advanced/CPU), puis relancez Monodon. » |
| Édition Home | `Win32_OperatingSystem.OperatingSystemSKU` ∈ SKU « Core » | « Monodon nécessite Windows 11 Pro, Entreprise ou Éducation. » + lien de documentation |
| Fonctionnalité manquante | API DISM (`DismGetFeatureInfo`) | « Un composant Windows doit être activé. Monodon va le faire ; un redémarrage sera nécessaire. » (bouton) |
...
| Hyperviseur non lancé (autre hyperviseur, `hypervisorlaunchtype off`, Monodon dans une VM sans virtualisation imbriquée) | HCS `HCS_E_HYPERV_NOT_RUNNING (0x80370102)` / `HCS_E_HYPERV_NOT_INSTALLED (0x80370101)` | « Le moteur de virtualisation Windows n'est pas démarré. Causes fréquentes : VirtualBox/VMware anciens, `bcdedit hypervisorlaunchtype` désactivé, exécution dans une machine virtuelle. » |
| Services `vmcompute`/`hns` arrêtés ou absents | SCM | Tentative de démarrage, sinon message + commande à donner à l'administrateur |
| Antivirus bloquant l'accès aux VHDX ou au binaire | `E_ACCESSDENIED` à la création, service tué | « Un logiciel de sécurité bloque Monodon. Ajoutez `%ProgramData%\Monodon` et `MonodonService.exe` aux exclusions. » |
| RAM insuffisante | < 4 Go libres | Avertissement, réduction automatique de la mémoire allouée |
| Service Monodon arrêté / désactivé | SCM | Démarrage à la demande depuis l'app (droit accordé à l'utilisateur interactif via ACL du service) |
...
├── crates/
│   ├── monodon-core/            modèles, erreurs à codes stables, traits `VmBackend`, `EngineBackend`,
│   │                          `HostShareProvider`, `NetworkProvider`, protocole RPC (types partagés hôte/agent)
│   ├── monodon-vm-hcs/          [windows] bindings HCS/HCN/virtdisk, document JSON, cycle de vie, orphelins
│   ├── monodon-hvsock/          [windows] `HvStream`/`HvListener` Tokio, GUID de service ↔ port vsock
│   ├── monodon-engine/          client `bollard`, flux logs/stats/events, exec hijacké, réécriture de chemins
│   ├── monodon-prereq/          [windows] détection/activation des prérequis, mapping HRESULT → codes
│   ├── monodon-service/         [windows] binaire service Windows : machine à états, relais pipe→hvsock,
│   │                          relais de ports, API de contrôle sur `\\.\pipe\monodon-control`
│   ├── monodon-agent/           [linux, musl statique] PID 1 invité : montages, dockerd, RPC, compose, arrêt
│   ├── monodon-ipc/             protocole app ↔ service (JSON-RPC sur named pipe), client et serveur
│   └── monodon-bench/           mesures : temps de démarrage, RAM, débit 9P (résultats copiés dans le README)
├── apps/desktop/              Tauri v2 (src-tauri/ + src/ React)
├── image/                     Dockerfiles noyau/rootfs/initrd, `monodon.config`, `manifest`, scripts
├── installer/                 hooks NSIS, scripts d'activation de fonctionnalités, enregistrement HvSocket
├── docs/                      décisions (ADR), mesures, dépannage
└── tests/                     tests d'intégration (gated `MONODON_IT=1`, Windows + Hyper-V)
```
...
- **Intégration VM** (`tests/vm_*.rs`, `#[ignore]` sauf `MONODON_IT=1`, exécution en Administrateur sur Windows avec Hyper-V) : création/démarrage/arrêt, rattachement après redémarrage du service, terminaison d'une orpheline, `Ping` HvSocket, `docker version` via le pipe, montage 9P aller-retour, **test de crash** (`HcsTerminateComputeSystem` pendant une écriture ext4, redémarrage, `fsck` propre, `dockerd` répond, volume intact), fuite de flux (ouverture/fermeture de 100 flux de logs).
...
- **Bancs de mesure** (`monodon-bench`) : démarrage à froid → API prête, RAM au repos à 5 min (`vmmem` + service + app), 9P vs volume nommé. Résultats versionnés dans `docs/measurements.md`.
...
- **NSIS** via le bundler Tauri (`installMode: perMachine`, donc élevé), hooks `installer/hooks.nsh` → `installer/setup.ps1` : vérification et activation des fonctionnalités (`Microsoft-Hyper-V`, `VirtualMachinePlatform`) avec drapeau « redémarrage requis » (code 3010 → `SetRebootFlag`) ; installation et démarrage de `MonodonService` (Automatique). Aucun enregistrement HvSocket dans le registre : toutes les connexions sont ouvertes par l'hôte. L'image (`vmlinuz`, `initrd.img`, `rootfs.vhd`, `manifest.json`) et `monodon-service.exe` sont installés **dans le dossier d'installation** ; le service cherche `image\` à côté de son exécutable avant `%ProgramData%\Monodon\image`. Les données (`data.vhdx`, `state.json`, `settings.json`, journaux) restent dans `%ProgramData%\Monodon`. Désinstallation : arrêt et suppression du service (qui arrête la machine et détache le réseau), **question explicite** avant de supprimer `%ProgramData%\Monodon` (réponse par défaut : conserver, y compris en mode silencieux).
...
- **Signature** : configuration séparée `apps/desktop/src-tauri/tauri.signed.conf.json` (`bundle.windows.signCommand` → `installer/sign.ps1`, `signtool` avec certificat OV/EV du magasin ou Azure Trusted Signing, horodatage RFC 3161) ; construire avec `npm run tauri build -- --config src-tauri/tauri.signed.conf.json`. Tauri signe alors `monodon.exe`, l'installeur et le désinstalleur ; `monodon-service.exe` étant une ressource, `sign.ps1` doit aussi lui être appliqué avant la construction (`installer/sign.ps1 -Path target\release\monodon-service.exe`). Sans certificat, SmartScreen avertira ; la réputation ne se construit qu'avec un certificat stable.
...
|---|---|---|
| **0a — Spike VM** | Dépôt, workspace, `monodon-vm-hcs` minimal : document JSON, création, démarrage d'un noyau (Alpine `linux-virt` en secours, WSL2 en cible) avec console série vers un pipe, arrêt, orphelins | Un noyau démarre et s'arrête depuis un test Rust ; besoins en privilèges confirmés ; champs HCS validés |
| **0b — Spike canaux** | `monodon-hvsock`, agent « echo », Plan9 monté depuis l'invité, micro-bancs 9P, relais pipe→hvsock | `Ping` aller-retour ; fichier écrit depuis l'invité visible dans l'Explorateur ; **chiffres 9P publiés** ; go/no-go sur 9P et Tokio |
| **1 — Image** | Pipeline noyau + rootfs VHDX + initrd reproductible, agent PID 1 minimal, `dockerd` démarre, `docker version` via le pipe | `docker run hello-world` (image locale) depuis le CLI Windows via `\\.\pipe\monodon` |
...
| R10 | Cohabitation avec Docker Desktop sur la même machine | Certaine sur la machine de test | Confusion de contexte `docker`, plages IP | Pipe distinct, contexte `monodon` non défini par défaut, réseau HNS distinct |

//! Emplacements sur disque du service. Tout vit sous `%ProgramData%\Solon` (lisible par le
...
impl Paths {
    pub fn default_root() -> PathBuf {
        ...
        let root = base.join("Solon");
        // Installation datant d'avant le renommage du projet (Monodon, 8 septembre 2026) : les données
        // (disque, réglages, journaux, autorité) sont reprises entrée par entrée, sans écraser ce que
        // Solon aurait déjà créé (l'installeur écrit son journal dans Solon\logs avant ce démarrage).
        let legacy = base.join("Monodon");

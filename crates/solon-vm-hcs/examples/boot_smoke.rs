...
//! marqueur `SOLON-INIT-OK` a été vu et que la machine s'est arrêtée toute seule.
...
fn main() {
    ...
    match HcsVm::terminate_orphans(None) {
        Ok(list) if list.is_empty() => out.line("aucune machine Solon orpheline"),
    ...
    loop {
        while let Ok(line) = rx.try_recv() {
            ...
            if line.contains("SOLON-INIT-START") {
                init_start = Some(t_start.elapsed().as_millis());
            }
            if line.contains("SOLON-INIT-OK") {

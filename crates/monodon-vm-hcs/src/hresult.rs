//! Traduction des HRESULT de HCS (et de quelques HRESULT Win32 génériques) en erreurs Monodon
//! à codes stables. Les constantes sont recopiées depuis `windows::Win32::Foundation`
//! (préfixe `HCS_E_`, plage `0x80370100`–`0x80370121`).

use monodon_core::{ErrorCode, MonodonError};
...
/// Classe un HRESULT en code d'erreur Monodon.
...
/// Construit une erreur Monodon à partir d'une erreur `windows` et du document de résultat
...
pub fn from_windows(
    ...
    MonodonError::new(classify(hresult), message).with_hresult(hresult)
}

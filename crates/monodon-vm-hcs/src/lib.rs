//! Pilotage de la machine Monodon via l'API Host Compute System (HCS) de Windows.
//!
//! Organisation :
...
//! - [`hresult`] : traduction des HRESULT HCS en codes d'erreur stables Monodon ;
...
use monodon_core::vm::{HostShare, VmConfig};
...
/// Valeur du champ `Owner` de tous les compute systems créés par Monodon. Sert à retrouver
/// les machines orphelines après un crash du service.
pub const OWNER: &str = "Monodon";

/// Machine Monodon pilotée par HCS.
pub struct HcsVm {
    id: String,
...
impl HcsVm {
    ...
    pub fn runtime_id(&self) -> Result<String> {
        ...
        props
            ...
            .map(str::to_owned)
            .ok_or_else(|| {
                MonodonError::internal(format!("RuntimeId absent des propriétés HCS : {props}"))
            })
    }
    ...
    /// Liste les compute systems appartenant à Monodon, quel que soit le processus créateur.
    ...
    /// Termine toutes les machines Monodon dont l'identifiant n'est pas `keep`.

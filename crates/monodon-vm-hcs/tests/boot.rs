...
//! ```powershell
//! $env:MONODON_IT = "1"
//! $env:MONODON_TEST_KERNEL = "C:\chemin\vmlinuz"
//! $env:MONODON_TEST_INITRD = "C:\chemin\initramfs.gz"   # doit afficher MONODON-INIT-OK puis poweroff
//! cargo test -p monodon-vm-hcs --test boot -- --nocapture
//! ```
//!
//! Sans `MONODON_IT=1`, le test est ignoré (il ne peut pas tourner sur un poste sans droits).
...
use monodon_vm_hcs::{HcsEventKind, HcsVm, new_vm_id};

fn integration_env() -> Option<(String, String)> {
    if std::env::var("MONODON_IT").ok().as_deref() != Some("1") {
        eprintln!("test ignoré : MONODON_IT=1 absent");
        return None;
    }
    let kernel =
        std::env::var("MONODON_TEST_KERNEL").expect("MONODON_TEST_KERNEL requis avec MONODON_IT=1");
    let initrd =
        std::env::var("MONODON_TEST_INITRD").expect("MONODON_TEST_INITRD requis avec MONODON_IT=1");
    Some((kernel, initrd))
}
...
fn demarre_puis_s_arrete_proprement() {
    ...
    assert!(
        ...
        "la machine doit être listée sous Owner=Monodon : {owned:?}"
    );

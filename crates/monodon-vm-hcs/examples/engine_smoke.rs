//! Bloc 1 : démarre l'image Monodon complète (noyau + initrd + rootfs.vhd + data.vhdx), attend le
//! moteur Docker, expose l'API sur `\\.\pipe\monodon` et exécute un conteneur depuis le CLI `docker`
...
use monodon_core::vm::{DiskAttachment, VmConfig};
use monodon_vm_hcs::{HcsVm, new_vm_id};
use windows::core::GUID;

const PIPE: &str = r"\\.\pipe\monodon";
const DOCKER_HOST: &str = "npipe:////./pipe/monodon";
...
impl Rpc {
    fn connect(vm: &GUID) -> std::io::Result<Self> {
        let s = monodon_hvsock::connect_with_retry(vm, 5000, Duration::from_secs(10))?;
...
/// Lance le CLI `docker` contre le pipe Monodon, avec un délai maximal (un flux qui ne se ferme
...
fn boot(out: &mut Out, config: &VmConfig) -> Result<BootResult, String> {
    ...
    while Instant::now() < deadline && r.engine_ready_ms.is_none() {
        if let Ok(line) = rx.recv_timeout(Duration::from_millis(100)) {
            ...
            if line.contains("MONODON-INITRD-OK") {
                r.initrd_ok_ms = Some(ms);
            } else if line.contains("MONODON-AGENT-READY") {
                r.agent_ready_ms = Some(ms);
                out.line(&format!("  | {line}"));
            } else if line.contains("MONODON-ENGINE-READY") {
                r.engine_ready_ms = Some(ms);
                out.line(&format!("  | {line}"));
            } else if line.contains("MONODON-") && line.contains("FAILED") {
                out.line(&format!("  | {line}"));
                break;
            } else if line.contains("[monodon-agent") || line.contains("AVERTISSEMENT") {
                out.line(&format!("  | {line}"));
            }
        }
    }
...
fn main() {
    ...
    if nobridge {
        cmdline.push_str(" monodon.nobridge");
    }
    let make_config = |id: String| VmConfig {
        id: id.clone(),
        name: "monodon-engine".into(),
        kernel: kernel.clone(),
        initrd: initrd.clone(),
        ...
        shares: vec![],
        serial_pipe: Some(format!(r"\\.\pipe\monodon-console-{}", &id[..8])),
    ...
    for cycle in 1..=2 {
        ...
        let relay = rt.spawn(monodon_hvsock::relay::serve_named_pipe(
            PIPE.into(),
            r.guid,
            5001,
        ));
        ...
        out.check(
            "docker version via \\\\.\\pipe\\monodon",
            ok,
            &format!("{txt} ({ms} ms)"),
        );
        let (ok, txt, ms) = docker(
        ...
        if cycle == 1 {
            if let Some(bb) = &busybox {
                ...
                let (ok, txt, ms) = docker(&["import", "-", "monodon/busybox:test"], Some(&tar));
                out.check(
        ...
        out.check(
            ...
            ok && txt.contains("monodon/busybox"),
        ...
        out.check("docker ps", ok && txt.contains("monodon-sleeper"), &txt);
        let (ok, _, ms) = docker(&["rm", "-f", "monodon-sleeper"], None);

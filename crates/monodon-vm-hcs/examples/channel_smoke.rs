...
//!
//! L'initrd doit lancer `monodon-agent` (spike) qui écoute sur vsock 5000.
...
fn main() {
    ...
    while Instant::now() < deadline {
        if let Ok(line) = rx.recv_timeout(Duration::from_millis(100)) {
            if line.contains("MONODON-AGENT-READY") {
                agent_ready = Some(t_start.elapsed().as_millis());
                break;
            }
            if line.contains("MONODON-AGENT-FAILED") || line.contains("monodon-agent]") {
    ...
    let t = Instant::now();
    let stream =
        match monodon_hvsock::connect_with_retry(&vm_guid, AGENT_PORT, Duration::from_secs(10)) {
            Ok(s) => s,
            Err(e) => {
    ...
    let tokio_ok = (|| -> std::io::Result<u128> {
        let std_stream = monodon_hvsock::connect_once(&vm_guid, AGENT_PORT)?;
    ...
    for l in console_lines
        ...
        .unwrap()
        .iter()
        .filter(|l| l.contains("monodon-agent") || l.contains("9p") || l.contains("MONODON"))
    {
        out.line(&format!("  | {l}"));
    }
    out.line(&format!(

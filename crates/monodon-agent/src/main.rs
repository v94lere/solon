//! Agent invité Monodon : PID 1 de la machine Linux.
//!
...
//!    (5001) et les ports publiés (5002). Protocole partagé : `monodon_core::protocol`.
//!
//! Hors Linux, le binaire n'a pas de sens : il s'arrête avec un message.

#[cfg(not(target_os = "linux"))]
fn main() {
    eprintln!("monodon-agent ne s'exécute que sous Linux (cible x86_64-unknown-linux-musl).");
    std::process::exit(1);
}
...
fn main() {
    ...
    std::thread::spawn(monodonfs::serve_pool);
    let ready_ms = t0.elapsed().as_millis();
    println!(
        "MONODON-AGENT-READY agent_ms={ready_ms} uptime_s={:.2}",
        system::uptime_secs()
    );
    rpc::serve(state);
}

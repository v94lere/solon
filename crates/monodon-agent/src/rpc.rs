//! Port de contrôle (vsock 5000) : requêtes/réponses JSON par lignes, protocole `monodon_core::protocol`.
...
use monodon_core::protocol::{Command, PORT_CONTROL, Request, Response};
...
pub fn serve(state: Arc<State>) -> ! {
    let listen_fd = match vsock::listen(PORT_CONTROL) {
        ...
        Err(e) => {
            ...
            println!("MONODON-AGENT-FAILED {e}");
    ...
    loop {
        ...
        let response = match request.command {
            ...
            Command::MountShare(req) => match system::mount_plan9(&req) {
                Ok(r) => {
                    // Même lecteur exposé aussi par monodonfs (FUSE), en parallèle du 9P.
                    crate::monodonfs::mount(&req.name);
                    Response::ok(id, r)
                }
                Err(e) => Response::err(id, e),
            },

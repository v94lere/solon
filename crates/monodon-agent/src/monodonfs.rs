//! Client FUSE « monodonfs » : expose les lecteurs Windows partagés sous `/mnt/host/<lettre>`
//! en interrogeant le serveur de fichiers du service (voir `monodon_core::fs`).
//!
...
use monodon_core::fs::{FS_MAX_IO, FsAttr, FsKind, FsRequest, FsResponse, PORT_FS, encode_frame};
...
pub fn serve_pool() {
    let listen_fd = match vsock::listen(PORT_FS) {
        Ok(fd) => fd,
        Err(e) => {
            log(&format!("monodonfs : {e}"));
            return;
        }
    };
    log(&format!(
        "monodonfs : en attente des connexions de l'hôte (vsock {PORT_FS})"
    ));
    loop {
...
fn call(req: &FsRequest, data: &[u8]) -> Result<(FsResponse, Vec<u8>), i32> {
    let Some(mut conn) = pool().take(Duration::from_secs(30)) else {
        log("monodonfs : aucune connexion de l'hôte depuis 30 s, opération refusée (EIO)");
        return Err(5);
    };
    ...
    match r {
        ...
        Err(e) => {
            log(&format!("monodonfs : connexion perdue : {e}"));
            drop(conn);
            Err(5)
        }
    }
...
/// Repli demandé par le service (`monodon.fs=9p` sur la ligne de commande du noyau) : le 9P de Windows
/// reste monté sur `/mnt/host` et monodonfs n'est pas monté.
pub fn legacy_mode() -> bool {
    std::fs::read_to_string("/proc/cmdline")
        .map(|c| c.split_whitespace().any(|kv| kv == "monodon.fs=9p"))
        .unwrap_or(false)
}
...
pub fn mount(drive: &str) {
    ...
    std::thread::spawn(move || {
        ...
        let options = [
            MountOption::FSName("monodonfs".into()),
            MountOption::Subtype("monodonfs".into()),
            MountOption::AllowOther,
            MountOption::NoAtime,
        ];
        log(&format!("monodonfs : montage de {target}"));
        if let Err(e) = fuser::mount2(fs, &target, &options) {
            log(&format!("monodonfs : montage {target} : {e}"));

...
use crate::MonodonError;
use crate::protocol::PortBinding;

pub const CONTROL_PIPE: &str = r"\\.\pipe\monodon-control";
/// Pipe où le service expose l'API Docker (compatible `docker -H npipe:////./pipe/monodon`).
pub const DOCKER_PIPE: &str = r"\\.\pipe\monodon";
/// Terminal dans la machine (relais vers le port vsock `PORT_SHELL`).
pub const SHELL_PIPE: &str = r"\\.\pipe\monodon-shell";
/// Exécution en flux (Compose) : relais vers le port vsock `PORT_EXEC`.
pub const EXEC_PIPE: &str = r"\\.\pipe\monodon-exec";
...
pub struct EngineSnapshot {
    ...
    #[serde(default)]
    pub reattached: bool,
    /// Le mandataire des domaines locaux (`*.monodon.local` → 127.0.0.1:80) est actif.
    #[serde(default)]
    pub local_domains: bool,
    /// Le mandataire HTTPS (127.0.0.1:443, autorité locale) est actif.
    #[serde(default)]
    pub local_domains_tls: bool,
    /// Conteneurs endormis par Monodon (identifiants), réveillés à la première connexion.
    #[serde(default)]
...
pub struct Settings {
    ...
    pub autostart: bool,
    /// Repli : partager les dossiers Windows par le 9P de Windows (ancien mécanisme) au lieu de monodonfs.
    #[serde(default)]
    pub legacy_file_sharing: bool,
    /// Réveil à la demande : endormir (pause) les conteneurs joints via Monodon restés inactifs.
...
/// Rapport des prérequis (produit par `monodon-prereq`).
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize, Default)]

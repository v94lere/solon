//! Protocole hôte ↔ agent invité, partagé par le service Windows et `monodon-agent`.
//!
...
pub struct MachineMetrics {
    ...
    /// Disque des données (`/var/lib/monodon`) en octets.
...
pub struct PortBinding {
    ...
    /// Nom du conteneur sans le `/` initial (domaines locaux `nom.monodon.local`).
    #[serde(default)]
    pub container_name: String,
    /// Étiquettes Compose, si le conteneur en vient (`service.projet.monodon.local`).
    #[serde(default)]
    pub compose_project: Option<String>,
    #[serde(default)]
    pub compose_service: Option<String>,

...
pub enum ErrorCode {
    ...
    /// Un logiciel de sécurité bloque l'accès aux fichiers ou aux binaires de Monodon.
...
/// Erreur principale de Monodon.
#[derive(Debug, Clone, PartialEq, Eq, thiserror::Error, Serialize, Deserialize)]
#[error("[{code:?}] {message}")]
pub struct MonodonError {
    pub code: ErrorCode,
    pub message: String,
...
impl From<std::io::Error> for MonodonError {
    fn from(e: std::io::Error) -> Self {
        Self::new(ErrorCode::Io, e.to_string())
    }
}

impl From<serde_json::Error> for MonodonError {
    fn from(e: serde_json::Error) -> Self {
        Self::new(ErrorCode::Internal, format!("JSON : {e}"))
    }
}

pub type Result<T> = std::result::Result<T, MonodonError>;

...
use monodon_core::protocol::NetworkConfig;
...
pub const NETWORK_NAME: &str = "Monodon";
/// Identifiant fixe du réseau Monodon : un seul réseau, retrouvé à chaque démarrage.
...
fn hcn_err(context: &str, e: &windows::core::Error, record: Option<String>) -> MonodonError {
    ...
    MonodonError::new(ErrorCode::HcsError, msg).with_hresult(hresult)
}
...
pub fn enumerate_networks() -> Result<Vec<String>> {
    ...
    let ids: Vec<String> = serde_json::from_str(&text)
        .map_err(|e| MonodonError::internal(format!("liste HNS illisible : {e}")))?;
    Ok(ids)
}

fn network_properties(id: &str) -> Result<serde_json::Value> {
    let guid =
        GUID::try_from(id).map_err(|e| MonodonError::internal(format!("GUID HNS {id} : {e}")))?;
    ...
    serde_json::from_str(&text)
        .map_err(|e| MonodonError::internal(format!("propriétés HNS illisibles : {e}")))
}
...
/// Supprime le réseau Monodon (et donc ses endpoints) s'il existe.
...
/// Crée (ou recrée) le réseau Monodon et un endpoint pour la machine. Renvoie tout ce que l'invité
...
pub fn delete_endpoint(endpoint_id: &str) -> Result<()> {
    let guid = GUID::try_from(endpoint_id)
        .map_err(|e| MonodonError::internal(format!("GUID endpoint {endpoint_id} : {e}")))?;
    let mut record = PWSTR::null();

//! Serveur de fichiers « monodonfs » : répond aux requêtes du client FUSE de l'invité sur les dossiers
//! Windows des lecteurs partagés. Voir `monodon_core::fs` pour le protocole.
//!
                ...
                match connect(vm_id).await {
                    Ok(stream) => {
                        if let Err(e) = serve_connection(&engine, stream).await {
                            tracing::debug!(i, "monodonfs : connexion terminée : {e}");
                        }
                    }
                    Err(e) => {
                        tracing::debug!(i, "monodonfs : connexion impossible : {e}");
                    }
                }
...
async fn connect(vm_id: GUID) -> io::Result<tokio::net::TcpStream> {
    let std_stream = tokio::task::spawn_blocking(move || {
        monodon_hvsock::connect_with_retry(&vm_id, PORT_FS, Duration::from_secs(5))
    })
    .await

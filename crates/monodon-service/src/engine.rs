...
use monodon_core::ipc::{EngineSnapshot, EngineState, ProvisionStep, ServiceEvent, Settings};
use monodon_core::protocol::{AgentEvent, Command, HealthReport, PROTOCOL_VERSION};
use monodon_core::vm::{DiskAttachment, NetworkAdapterConfig, VmConfig};
use monodon_core::{ErrorCode, MonodonError, Result};
use monodon_vm_hcs::{HcsEventKind, HcsVm};
...
pub const CONSOLE_PIPE: &str = r"\\.\pipe\monodon-console";
...
struct Running {
    ...
    shares: std::collections::HashMap<String, monodon_core::vm::HostShare>,
}
...
struct Inner {
    ...
    /// Domaines locaux `*.monodon.local` → port hôte (partagé avec le mandataire HTTP).
...
impl Engine {
    ...
    /// Recalcule la table `*.monodon.local` et le bloc du fichier `hosts` d'après les conteneurs en marche.
    async fn update_domains(&self, endpoints: &[monodon_core::protocol::ContainerEndpoint]) {
            ...
            Ok(running) => {
                ...
                *self.inner.running.lock().await = Some(running);
                self.set(|s| {
                    ...
                    s.docker_pipe = Some(monodon_core::ipc::DOCKER_PIPE.into());
                });
                Ok(())
            }
            Err(e) => {
    ...
    async fn provision(&self) -> Result<Running> {
        ...
        let prereq = tokio::task::spawn_blocking(monodon_prereq::check)
            .await
            .map_err(|e| MonodonError::internal(e.to_string()))?;
        if !prereq.ok {
        ...
        let image = tokio::task::spawn_blocking(move || ResolvedImage::load_verified(&image_dir))
            .await
            .map_err(|e| MonodonError::internal(e.to_string()))?
            .map_err(|e| MonodonError::new(ErrorCode::ImageCorrupted, e))?;
        ...
        let vm_id = monodon_vm_hcs::new_vm_id();
        ...
        if !shares.is_empty() {
            let list: Vec<String> = shares
                ...
                .collect();
            cmdline.push_str(&format!(" monodon.shares={}", list.join(",")));
        }
        if settings.legacy_file_sharing {
            cmdline.push_str(" monodon.fs=9p");
        }
        ...
        let guid = GUID::try_from(runtime_id.as_str())
            .map_err(|e| MonodonError::internal(format!("RuntimeId : {e}")))?;
    ...
    async fn attach(
        ...
        {
            // Serveur de fichiers monodonfs (FUSE côté invité) : ouvre ses connexions vers l'agent.
            let engine = self.clone();
            tasks.push(tokio::spawn(async move {
        ...
        tasks.push(tokio::spawn(async move {
            if let Err(e) = monodon_hvsock::relay::serve_named_pipe_with_sddl(
                monodon_core::ipc::EXEC_PIPE.into(),
                guid,
                monodon_core::protocol::PORT_EXEC,
                Some(monodon_hvsock::relay::DOCKER_PIPE_SDDL),
            )
            .await
            {
        ...
        tasks.push(tokio::spawn(async move {
            if let Err(e) = monodon_hvsock::relay::serve_named_pipe_with_sddl(
                monodon_core::ipc::SHELL_PIPE.into(),
                guid,
                monodon_core::protocol::PORT_SHELL,
                Some(monodon_hvsock::relay::DOCKER_PIPE_SDDL),
            )
            .await
            {
    ...
    pub async fn exec(
        ...
        command: String,
        timeout_s: u64,
    ) -> Result<monodon_core::protocol::ExecResult> {
        let agent = self
            .inner
            .running
            .lock()
            ...
            .map(|r| r.agent.clone())
            .ok_or_else(|| {
                MonodonError::new(ErrorCode::EngineUnreachable, "le moteur n'est pas démarré")
            })?;
        agent
            .call_typed(
                Command::Exec {
    ...
    pub async fn ensure_share(&self, host_path: &str) -> Result<monodon_core::ipc::ShareInfo> {
        let (drive, guest_path) =
            monodon_core::ipc::guest_path_for(host_path).ok_or_else(|| {
                MonodonError::new(
                    ErrorCode::Io,
        ...
        // Le 9P de Windows est monté en secours sous /mnt/host9p ; /mnt/host est servi par monodonfs
        ...
        let mounted_now = if running.shares.contains_key(&drive) {
            ...
            let share = monodon_core::vm::HostShare {
                name: drive.clone(),
            ...
            running
                .agent
                .call(
                    Command::MountShare(monodon_core::protocol::MountShareRequest {
                        name: drive.clone(),
                        port,
    ...
    pub async fn list_shares(&self) -> Vec<monodon_core::vm::HostShare> {
        self.inner
            .running
            .lock()
            .await
    ...
    pub async fn metrics(&self) -> Result<monodon_core::protocol::MachineMetrics> {
        let agent = self
            .inner
            .running
            .lock()
                ...
                Ok(n) => {
                    ...
                    while let Some(pos) = pending.iter().position(|&b| b == b'\n') {
                        ...
                        let text = String::from_utf8_lossy(&line)
                            .trim_end_matches(['\r', '\n'])
                            .to_owned();
                        if text.contains("[monodon-agent")
                            || text.contains("MONODON-")
...
fn boot_shares(drives: &[String]) -> Vec<monodon_core::vm::HostShare> {
    drives
        .iter()
        .enumerate()
        .filter_map(|(i, drive)| {
            ...
            Some(monodon_core::vm::HostShare {
                name: drive.clone(),

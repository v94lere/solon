...
//!
//! ```text
//! monodon-service run                 lancé par le Gestionnaire de services (ne pas appeler à la main)
//! monodon-service console [--start]   premier plan, journaux sur la sortie d'erreur (Administrateur)
//! monodon-service install | uninstall crée / supprime le service Windows « MonodonService »
//! monodon-service status | start | stop [--force] | restart | prereq | watch | version
//!                                   client du canal de contrôle (aucun droit particulier)
//! ```
//! Variables : `MONODON_ROOT` (défaut `%ProgramData%\Monodon`), `MONODON_IMAGE_DIR` (défaut `<root>\image`).
...
use monodon_service::engine::{Engine, EngineConfig};
use monodon_service::paths::Paths;
use monodon_service::settings;
use tokio::io::{AsyncBufReadExt, AsyncWriteExt, BufReader};

const SERVICE_NAME: &str = "MonodonService";
const SERVICE_DISPLAY: &str = "Monodon";
const SERVICE_DESCRIPTION: &str =
    "Monodon container engine: owns the Linux machine and exposes the Docker API.";

fn engine_config() -> EngineConfig {
    let root = std::env::var_os("MONODON_ROOT")
        .map(PathBuf::from)
        .unwrap_or_else(Paths::default_root);
    let paths = Paths::new(root);
    // Image : variable d'environnement, sinon `image\` à côté de l'exécutable (installeur), sinon
    // `%ProgramData%\Monodon\image`.
    let beside_exe = std::env::current_exe()
        .ok()
        .and_then(|p| p.parent().map(|d| d.join("image")))
        .filter(|d| d.join("manifest.json").is_file());
    let image_dir = std::env::var_os("MONODON_IMAGE_DIR")
        .map(PathBuf::from)
        .or(beside_exe)
...
fn init_logging(
    ...
    let file_appender = tracing_appender::rolling::daily(paths.logs_dir(), "monodon-service.log");
...
async fn run_core(
    ...
    let ipc = tokio::spawn(monodon_service::ipc::serve(
        engine.clone(),
...
fn usage() -> ! {
    eprintln!(
        "usage : monodon-service <run|console [--start]|install|uninstall|status|start|stop [--force]|restart|prereq|watch [--follow]|version>"
    );
    std::process::exit(2);
}
...
fn main() {
    ...
    let result: Result<(), String> = match cmd {
        ...
        "uninstall" => uninstall(),
        "version" => {
            println!("monodon-service {}", env!("CARGO_PKG_VERSION"));
            Ok(())
        }
        "share" => {
            let rt = tokio::runtime::Runtime::new().expect("runtime");

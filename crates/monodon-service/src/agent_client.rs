...
use monodon_core::protocol::{AgentEvent, Command, PORT_CONTROL, PORT_EVENTS, Request, Response};
...
async fn connect(vm: &GUID, port: u32, timeout: Duration) -> Result<TcpStream> {
    let vm = *vm;
    let std_stream =
        tokio::task::spawn_blocking(move || monodon_hvsock::connect_with_retry(&vm, port, timeout))
            .await
...
impl AgentClient {
    ...
    pub async fn call(&self, command: Command, timeout: Duration) -> Result<serde_json::Value> {
        ...
        io.1.write_all(line.as_bytes())
            .await
            .map_err(|e| MonodonError::new(ErrorCode::AgentUnreachable, e.to_string()))?;
        ...
        if response.id != id {
            return Err(MonodonError::internal(format!(

...
*English version: [README.md](README.md).* Monodon s'appelait **Solon** jusqu'au 8 septembre 2026 ; les installations existantes sont reprises automatiquement (dossier de données, autorité de certification, entrées du fichier hosts).

**Monodon est un gestionnaire de conteneurs autonome pour Windows**, dans l'esprit d'OrbStack : une
installation, aucun prérequis logiciel à installer soi-même, et un moteur Docker complet qui démarre
en quelques secondes dans une machine minuscule et invisible, gérée entièrement par Monodon.

Monodon **n'est pas** une interface pour un Docker Desktop déjà installé. Il remplace Docker Desktop :
...
- Expose l'API Docker sur `\\.\pipe\monodon` : l'interface Monodon et le CLI `docker` que vous avez déjà
  fonctionnent (`docker -H npipe:////./pipe/monodon ps`, ou un contexte `docker context create`).
...
| Mémoire | 8 Go recommandés (4 Go minimum). Monodon alloue 2 Go au moteur par défaut, ajustable dans les réglages. |
| Disque | ~400 Mo pour Monodon et son image Linux, plus un disque de données dynamique (64 Go maximum par défaut, occupé à la demande). |
| Composants Windows | « Hyper-V » et « Plateforme de machine virtuelle ». L'installeur les active ; un redémarrage peut être demandé. |

Monodon cohabite avec WSL2 et Docker Desktop : il n'utilise ni leur pipe, ni leurs réseaux, ni leur
...
`Monodon_0.1.0_x64-setup.exe` (80 Mo, produit par `npm run tauri build` dans `apps/desktop`, sorti dans
`target/release/bundle/nsis/`). Il demande l'élévation une fois, puis : active les composants Windows
nécessaires (un redémarrage peut être demandé), installe le service `MonodonService`, copie l'image Linux et
crée le raccourci. Installation silencieuse : `Monodon_0.1.0_x64-setup.exe /S`.
...
Désinstallation : Paramètres → Applications → Monodon. Le désinstalleur arrête le moteur, retire le service et
**demande** avant de supprimer vos images et volumes (`%ProgramData%\Monodon`) ; par défaut ils sont conservés.

### Depuis les sources (développeurs)

Prérequis : Rust stable (≥ 1.85), Node 22, et **Monodon lui-même** installé (l'image Linux du moteur se construit
dans un conteneur Monodon : pas besoin de WSL). Une session Windows **administrateur** sert seulement au mode console.
...
# 2. Image Linux (noyau réutilisé + système racine Alpine + initrd), construite dans un conteneur Monodon (~15 s)
...
- **Réveil à la demande** : un conteneur joint via Monodon (adresse `monodon.local` ou port publié) qui ne reçoit plus
...
- **Debug shell** (onglet de la fiche) : un shell d'outils (bash, curl, dig, ps, strace, tcpdump, jq, vim…) qui
  ...
  modifié dans le conteneur ; la boîte à outils (`monodon-debug`) se construit au premier usage.
...
- **Des adresses qui marchent, toujours** : chaque conteneur en marche est joignable sur
  `https://<nom>.monodon.local` (et `https://<service>.<projet>.monodon.local` pour Compose), **qu'il publie un port
  ou non**. Monodon lit le port que l'image expose (80, 8080, 3000, 8069…), tient à jour un bloc dans le fichier
  `hosts` de Windows, et route les requêtes depuis `127.0.0.1:80` (HTTP) et `127.0.0.1:443` (HTTPS) d'après le nom
  demandé. Le HTTPS repose sur une autorité de certification locale « Monodon Local CA », créée à la première
...
- **Notifications Windows** : conteneur arrêté avec une erreur (hors actions faites dans Monodon), moteur en échec
...
- **Réglages** : langue (anglais par défaut, français), apparence (clair, sombre, Windows), couleur d'accent (bleu
  Monodon ou couleur de Windows), mémoire et processeurs du moteur (par défaut
...
- **Barre des tâches** : un clic sur l'icône ouvre le menu : état du moteur, nombre de conteneurs en
  marche, **chaque conteneur avec Démarrer / Redémarrer / Arrêter**, ouvrir Monodon, démarrer ou arrêter le
  moteur, quitter. Dans la langue de l'interface. Fermer la fenêtre laisse Monodon actif dans la barre des tâches.

### CLI `docker` et `docker compose` intégrés

L'installeur place `C:\Program Files\Monodon\bin` en tête du PATH. Il contient le **CLI Docker officiel** et le
**plugin Compose** (Apache-2.0, versions dans `bin\NOTICE-third-party.txt`), derrière un petit lanceur `docker.exe`
qui les dirige vers le moteur Monodon. Dans un **nouveau** terminal :
...
> `docker logout` ou utilisez un `DOCKER_CONFIG` vide pour le vérifier ; ce n'est pas lié à Monodon.

## Dossiers Windows partagés : comment ça marche

Les dossiers Windows montés dans les conteneurs (`-v C:\...`, projets Compose) passent par **monodonfs**, le système de
fichiers de Monodon : un serveur côté Windows, un client côté Linux, et un protocole qui ramène tout un dossier en une
...
- Pour les dépendances et les bases de données, préférez toujours des **volumes Docker** : ils vivent sur le disque de
  Monodon, à vitesse native (20 à 50 ms pour les mêmes 5 000 fichiers).
...
`%ProgramData%\Monodon\logs` (service) et se copient depuis l'écran d'erreur.
...
| `WINDOWS_FEATURE_MISSING` | Hyper-V ou Plateforme de machine virtuelle désactivés | Réinstaller Monodon (l'installeur les active) ou, en PowerShell administrateur : `Enable-WindowsOptionalFeature -Online -FeatureName Microsoft-Hyper-V,VirtualMachinePlatform -All`, puis redémarrer. |
...
| `HYPERVISOR_NOT_RUNNING` | Hyperviseur Windows non démarré | Désinstaller les anciens VirtualBox/VMware (< 6.1 / < 15.5), vérifier `bcdedit /enum` (`hypervisorlaunchtype Auto`), ne pas exécuter Monodon dans une VM sans virtualisation imbriquée. |
| `HOST_COMPUTE_SERVICE_UNAVAILABLE` | Service `vmcompute` ou `hns` arrêté ou absent | Redémarrer Windows ; sinon réinstaller Monodon. |
| `BLOCKED_BY_SECURITY_SOFTWARE` | Antivirus / EDR bloque les disques ou le service | Ajouter `%ProgramData%\Monodon` et le dossier d'installation aux exclusions. |
| `INSUFFICIENT_PRIVILEGES` | Le service ne tourne pas avec les droits attendus | Réinstaller Monodon (le service doit tourner en LocalSystem). |
| `IMAGE_CORRUPTED` | Fichiers du moteur absents ou empreinte SHA-256 invalide | Réinstaller Monodon. |
| `DATA_DISK_ERROR` | `data.vhdx` impossible à créer ou ouvrir | Vérifier l'espace disque et les exclusions antivirus ; en dernier recours renommer `%ProgramData%\Monodon\data.vhdx` (perte des données Docker). |
| `VM_BOOT_TIMEOUT`, `AGENT_UNREACHABLE`, `ENGINE_UNREACHABLE` | La machine ne répond pas | Redémarrer le moteur ; consulter `monodon-service.log` ; signaler avec le journal. |
| Pas d'accès réseau depuis les conteneurs | VPN d'entreprise ou conflit de plage IP | Monodon choisit une plage libre parmi `172.30.0.0/24`… et fixe le MTU à 1400 ; certains VPN (AnyConnect, GlobalProtect) bloquent tout de même le trafic des cartes virtuelles : désactiver le VPN pour tester, puis signaler. |
| « Le service Monodon n'est pas en cours d'exécution » | Service arrêté | `sc start MonodonService` en administrateur, ou réinstaller. |

Coupure de courant ou arrêt brutal : au démarrage suivant, Monodon vérifie et répare le disque de données
...
Apache 2.0, voir `LICENSE` et `NOTICE`. Monodon embarque des logiciels tiers (CLI Docker, Docker Compose, un noyau
